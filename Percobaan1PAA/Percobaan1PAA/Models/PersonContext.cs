using Npgsql;
using Percobaan1PAA.Helper;

namespace Percobaan1PAA.Models
{
    public class PersonContext
    {
        ...
        public List<Person> ListPerson()
        {
            ...
            string query = string.Format(@"SELECT id_person, nama, alamat, email FROM person order by id_person;");
        ...
        {
            string query = @"UPDATE person SET nama = @nama, alamat = @alamat, email = @email WHERE id_person = @id_person";
            sqlDBhelper db = new sqlDBhelper(this.__constr);
            try
            {
                NpgsqlCommand cmd = db.getNpgsqlCommand(query);
        ...
        {
            string query = @"DELETE FROM person WHERE id_person = @id_person";
            sqlDBhelper db = new sqlDBhelper(this.__constr);
            try
            {
                NpgsqlCommand cmd = db.getNpgsqlCommand(query);
